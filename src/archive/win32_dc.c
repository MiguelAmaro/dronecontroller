...
// TODO(MIGUEL): The project directory should be establis at startup and stored
//               all paths used for logging/outputing data should use that path.
  ...
  if(RegisterClass(&WindowClass)) 
  {
    ...
    if(Window)
    {
...
#endif
#if RENDERER_D3D11
      d3d11_renderer *Renderer = RendererInit(Window,
                                              g_Platform.WindowWidth,
                                              g_Platform.WindowHeight,
                                              &Constraints);
#endif
      ...
      while(!g_Platform.QuitApp)
      {
        ...
        Device.Connected    = g_SerialPortDevice.Connected;
        Device.State        = g_SerialPortDevice.State;
        Device.PacketQueues = &(g_SerialPortDevice.PacketQueues);
        ...
        if(AppCode.Update)
        {
          AppCode.Update(&g_Platform, &Device, &Renderer-> RenderData);
        }
