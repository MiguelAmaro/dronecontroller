...
u32 
Shader_create_program(readonly u8* vertexShaderSource, readonly u8* fragmentShaderSource)
{
...
void 
Shader_get_source(u32 *ShaderProgram, readonly u8 *path)
{
    //printf("\n\n************************\n  %s  \n ******************** \n", path);
    FILE *File = (void *)0x00;
    File = fopen(path, "r");
    ...
    *ShaderProgram = Shader_create_program(Shader + VSpos, Shader + FSpos);
