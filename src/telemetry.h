...
fn b32 TelemetryQueueFull(telem_packet_queues *Queues, telem_queue Direction)
{
  b32 Result = 0;
  Result = (&Queues->QueueCount[Direction] >= &Queues->QueueMaxCount[Direction]);
  return Result;
}
fn b32 TelemetryQueueEmpty(telem_packet_queues *Queues, telem_queue Direction)
{
  b32 Result = 0;
  Result = (&Queues->QueueCount[Direction] <= 0);
  return Result;
}
fn void TelemetryEnqueuePacket(telem_packet_queues *Queues, telem_queue Direction, telem_packet Packet)
{
...
fn telem_packet TelemetryDequeuePacket(telem_packet_queues *Queues,
